...
def ancestry_optimized(article1, article2):
    ...
    asyncio.run(search_optimized(qid, sqid, year, name, search_name))
    print('\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    print(str(amount) + ' royals were examined.')

    print(str(round(time.monotonic() - started_at, 2)) + 's spent at total.')
...
async def search_optimized(qid, sqid, year, name, search_name):
    global found
    ...
    while len(buffer) != 0:  # BFS
        ...
        # create a queue to parallelize the process even more
        queue = asyncio.Queue()
        # create as many threads as many task we have
        tasks = []
        for qid in texts:
            queue.put_nowait((qid, texts[qid]))
            task = asyncio.create_task(thread(queue, year, buffer, sqid))
            tasks.append(task)
...
async def thread(queue, year, buffer, sqid):
    # Get a "work item" out of the queue.
    global found
    if found is not None:
        return end_thread(queue)
    [qid, text] = await queue.get()
    ...
    # Notify the queue that the thread has been processed
    return end_thread(queue)
